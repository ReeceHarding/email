...
const TEST_USER = {
  ...
  stripeSubscriptionId: "sub_test_123" // User has an active subscription
}
...
async function testWebhookSubscriptionDeleted() {
  try {
    ...
    if (res.status === 200 && responseText === "Webhook processed") {
      // Verify database update - subscription should be removed
      ...
      if (
        ...
        console.log("Test passed: Subscription deleted webhook handled correctly")
